...
namespace VehicleInsuranceSem3.Controllers
{
    public class PaypalPaymentController : Controller
    {
        ...
        private Payment CreatePayment(APIContext apiContext, string redirectUrl)
        {
            ...
            {
                name = "1 - " + checkout.CustomerPolicy.Policy.policy_number,
                currency = "USD",
                price = checkout.CustomerPolicy.total_payment.ToString(),
                quantity = "1",
                sku = ""
            });
            var payer = new Payer()
            ...
            var details = new Details()
            {
                tax = "0",
                shipping = "0",
                subtotal = checkout.CustomerPolicy.total_payment.ToString()
            };
            //Final amount with details  
            var amount = new Amount()
            {
                currency = "USD",
                total = checkout.CustomerPolicy.total_payment.ToString(), // Total must be equal to sum of tax, shipping and subtotal.  
                details = details
            };
        ...
        public void AddCustomerPolicy()
        {
            ...
            {
                ctx.Vehicle_Info.Add(checkout.Vehicle);
                ctx.SaveChanges();
                ctx.Customer_Policy.Add(checkout.CustomerPolicy);
                ctx.SaveChanges();
            }
        }
    }
