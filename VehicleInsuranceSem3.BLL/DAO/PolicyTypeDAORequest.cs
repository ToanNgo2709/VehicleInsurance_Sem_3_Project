   ...
   public class PolicyTypeDAORequest : ICrudFeature<PolicytypeViewModel>
    {
        ...
        {
            var q = context.Policy_Type.Select(d => new PolicytypeViewModel { id = d.id , name = d.name , active =d.active }).ToList();
            return q;
        ...
        {
            var q = context.Policy_Type.Where(d => d.id == id).Select(d => new PolicytypeViewModel { id = d.id, name = d.name, active = d.active }).FirstOrDefault();
            return q;
        }
        ...
        {
            var q = context.Policy_Type.Select(d => new PolicytypeViewModel { id = d.id, name = d.name, active = d.active }).OrderBy(d => d.id).Skip((page - 1) * row).Take(row).ToList();
            return q;
        }

        public List<PolicytypeViewModel> Search(int page, int row, string keyword)
        {
            var CountItem = context.Policy_Type.Where(d => d.name.ToLower().Contains(keyword.ToLower())).Count();
            ...
            var q = context.Policy_Type.Where(d => d.name.ToLower().Contains(keyword.ToLower())).Select(d => new PolicytypeViewModel { id = d.id, name = d.name, active = d.active }).OrderBy(d => d.id).Skip((page - 1) * row).Take(row).ToList();
            return q;
